...
import {
  Calendar, Users, FileText, Trophy, ChevronDown, Sparkles, TrendingUp,
} from 'lucide-react'
...
export default function ArchiveClient() {
  ...
  return (
    <div className="archive-page">
      <PageTopBar title="SIH Archive" count={`${EDITIONS.length} editions`} className="archive-topbar" />

      <div className="container">
        <section className="archive-hero">
          <span className="kicker mono">// Editions Archive</span>
          <h1>Nine editions. One movement.</h1>
          <p>From a single-track hackathon in 2017 to the world&rsquo;s largest open innovation platform, here is how Smart India Hackathon grew year after year.</p>
        </section>
        ...
        <section className="archive-timeline">
          <div className="archive-rail" aria-hidden="true"><div className="archive-rail-fill" /></div>

          {ordered.map((e, i) => {
            const isOpen = expanded === e.year
            return (
              <article className={`archive-item archive-item-${e.accent}`} key={e.year} style={{ animationDelay: `${i * 70}ms` }}>
                <div className="archive-node"><span className="mono">{e.year}</span></div>
                ...
                <div className="archive-card">
                  ...
                  <div className={`archive-body ${isOpen ? 'open' : ''}`}>
                    ...
                    <ul className="archive-highlights">
                      {e.highlights.map((h) => (
                        <li key={h}><Sparkles size={13} /><span>{h}</span></li>
                      ))}
                    </ul>
        ...
        <div className="archive-cta">
          ...
          <Link className="btn btn-primary" href="/problems">Explore SIH 2026</Link>
        </div>
      </div>
    </div>
  )
