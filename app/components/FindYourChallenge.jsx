...
import {
  ...
  Building2, Award, Plus, RotateCcw, Target,
} from 'lucide-react'
...
export default function FindYourChallenge({ open, onClose, problems, onSelectProblem }) {
  ...
  const matched = useMemo(() => {
    ...
    return [...problems]
      ...
      .slice(0, 3)
      .map(({ p, score }) => ({ problem: p, match: Math.min(100, Math.round((score / 100) * 100)) }))
  }, [problems, answers])
  ...
  const reset = () => { setStep(0); setAnswers({ theme: null, type: null, difficulty: null }) }

  return (
    <div className="fycm-backdrop" onClick={onClose}>
      <div className="fycm-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        <button className="fycm-x" onClick={onClose} aria-label="Close"><X size={20} /></button>

        <div className="fycm-progress">
          {STEP_LABELS.map((label, i) => (
            <div key={label} className={`fycm-step-pip ${i === step ? 'on' : ''} ${i < step ? 'done' : ''}`}>
        ...
        <div className="fycm-body">
          {step === 0 && (
            <div className="fycm-panel">
              ...
              <h2>What kind of problem excites you?</h2>
            ...
            <div className="fycm-panel">
              ...
              <div className="fycm-option-list">
                {TYPE_OPTIONS.map((o) => {
                  ...
                  return (
                    <button key={o.value} className={`fycm-option ${on ? 'on' : ''}`}
          ...
          {step === 3 && (
            <div className="fycm-panel">
              ...
              <p className="fycm-sub">Top 3 of {problems.length} problem statements, ranked by your answers.</p>
                  ...
                  <article className="fycm-result" key={p.id} style={{ animationDelay: `${i * 80}ms` }}>
                    ...
                    <div className="fycm-result-main">
                      ...
                      <div className="fycm-result-actions">
                        <button className="btn btn-sm btn-primary" onClick={() => { onClose(); onSelectProblem?.(p) }}>
        ...
        {step < 3 && (
          <div className="fycm-nav">
            ...
            <span className="fycm-nav-hint mono">{step + 1} / {STEP_LABELS.length - 1}</span>
