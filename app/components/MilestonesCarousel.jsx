...
import {
  Users, GraduationCap, Building2, FileText, Rocket, ChevronLeft, ChevronRight,
} from 'lucide-react'
    ...
    <section className="section milestones-section" id="milestones">
      <div className="container">
              ...
              return (
                <div className="milestone-slide" key={m.label}>
                  <div className={`ms-card ms-accent-${m.accent}`}>
                    <div className="ms-icon-wrap"><Icon size={24} strokeWidth={1.6} /></div>
        ...
        <div className="milestones-controls">
          <button className="ms-nav" onClick={prev} aria-label="Previous milestone"><ChevronLeft size={18} /></button>
          <div className="ms-dots">
            {MILESTONES.map((_, i) => (
              <button key={i} className={`ms-dot ${i === index ? 'on' : ''}`}
                onClick={() => setIndex(i)} aria-label={`Go to milestone ${i + 1}`} />
            ))}
          </div>
          <button className="ms-nav" onClick={next} aria-label="Next milestone"><ChevronRight size={18} /></button>
        </div>
      </div>
    </section>
  )
