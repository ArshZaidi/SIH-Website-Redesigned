const SIH_SPOC_API = 'https://sih.gov.in/api/spoc'
...
export async function fetchSPOCs() {
  if (typeof window === 'undefined') return FALLBACK_SPOCS
  try {
...
export function searchSPOCs(list, query) {
  ...
  return list.filter((s) =>
    [s.institute, s.code, s.state, s.spocName, s.email].join(' ').toLowerCase().includes(q),
  )
}
